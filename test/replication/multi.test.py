import sys
import os
from lib.tarantool_server import TarantoolServer
import yaml

REPLICA_N = 3
ROW_N = REPLICA_N * 20

##

# master server
master = server
master.admin("fiber = require('fiber')")
master.admin("box.schema.user.grant('guest', 'replication')")
master.admin("box.schema.user.grant('guest', 'execute', 'universe')")

print '----------------------------------------------------------------------'
print 'Bootstrap replicas'
print '----------------------------------------------------------------------'

# Start replicas
master.id = master.get_param('server')['id']
master_lsn = master.get_lsn(master.id)
cluster = [ master ]
for i in range(REPLICA_N - 1):
    server = TarantoolServer(server.ini)
    server.script = 'replication/replica.lua'
    server.vardir = os.path.join(server.vardir, 'replica', str(master.id + i))
    server.rpl_master = master
    server.deploy()
    # Wait replica to fully bootstrap.
    # Otherwise can get ACCESS_DENIED error.
    server.wait_lsn(master.id, master_lsn)
    cluster.append(server)

# Make a list of servers
sources = []
for server in cluster:
    sources.append(yaml.load(server.admin('box.cfg.listen', silent = True))[0])
    server.id = server.get_param('server')['id']

print 'done'

print '----------------------------------------------------------------------'
print 'Make a full mesh'
print '----------------------------------------------------------------------'

# Connect each server to each other to make full mesh
for server in cluster:
    server.iproto.py_con.eval("box.cfg { replication_source = ... }", [sources])

# Wait connections to establish
for server in cluster:
    for source in sources:
        server.iproto.py_con.eval("""
            while box.info.replication.source[...].status ~= 'follow' do
                fiber.sleep(0.01)
            end;""", source)
        print 'server', server.id, "connected"
    server.admin("box.info.vclock")

print 'done'

print '----------------------------------------------------------------------'
print 'Test inserts'
print '----------------------------------------------------------------------'

print 'Create a test space'
master.admin("_ = box.schema.space.create('test')")
master.admin("_ = box.space.test:create_index('primary')")
master_lsn = master.get_lsn(master.id)
# Wait changes to propagate to replicas
for server in cluster:
    server.wait_lsn(master.id, master_lsn)
    print 'server', server.id, 'is ok'
print

print 'Insert records'
for i in range(ROW_N):
    server = cluster[i % REPLICA_N]
    server.admin("box.space.test:insert{%d, %s}" % (i, server.id), silent = True)
print 'inserted %d records' % ROW_N
print

print 'Synchronize'
for server1 in cluster:
    for server2 in cluster:
        server1.wait_lsn(server2.id, server2.get_lsn(server2.id))
    print 'server', server.id, 'done'
print 'done'
print

print 'Check data'
for server in cluster:
    cnt = yaml.load(server.admin("box.space.test:len()", silent = True))[0]
    print 'server', server.id, 'is', cnt == ROW_N and 'ok' or 'not ok'
print 'Done'
print

print '----------------------------------------------------------------------'
print 'Test "connect" and "disconnecting" states'
print '----------------------------------------------------------------------'

# Set unreachable remote address
master.admin("box.cfg { replication_source = '0' }")
master.admin("server_uri, rinfo = next(box.info.replication.source)")
master.admin("rinfo.status == 'connect' or rinfo.status == 'disconnected'")
master.admin("box.cfg { replication_source = {} }")
# Check that fiber was killed
master.admin("fiber.info()[rinfo.fid] == nil")
# Restore connections
master.iproto.py_con.eval("sources = ...", [sources])
master.admin("box.cfg { replication_source = sources }")

print '----------------------------------------------------------------------'
print 'Test "off" state'
print '----------------------------------------------------------------------'

print 'kill reader fiber'
master.admin("server_uri, info = next(box.info.replication.source); fiber.kill(info.fid)")

print 'check that status changed to off'
master.admin("box.info.replication.source[server_uri].status == 'off'")

print 're-configuration restartes reader'
master.iproto.py_con.eval("sources = ...", [sources])
master.admin("box.cfg { replication_source = sources }")

print 'wait to reconnect'
master.admin("while box.info.replication.source[server_uri].status ~= 'follow' do fiber.sleep(0.01) end")
print 'done'

print '----------------------------------------------------------------------'
print 'Test "stopped" state'
print '----------------------------------------------------------------------'

print 'Break up full mesh'
for server in cluster:
    server.admin("box.cfg{ replication_source = {} }", silent = True)
print 'done'
print

master.admin("box.info.replication.source")

print 'Insert duplicate records'
cluster[0].admin("box.space.test:insert{100500, 'bad record'}")
cluster[1].admin("box.space.test:insert{100500, 'bad record'}")
print 'done'
print

print 'Restore full mesh'
for server in cluster:
    server.iproto.py_con.eval("box.cfg { replication_source = ... }", [sources])
print 'done'
print

master.admin("server_uri = '%s'" % sources[1], silent = True)

print 'Wait for "stopped" state'
master.iproto.py_con.eval("""
    while box.info.replication.source[server_uri].status ~= 'stopped' do
        fiber.sleep(0.01)
    end""")
master.admin("box.info.replication.source[server_uri].status")
master.admin("box.info.replication.source[server_uri].message")
print 'done'
print

print
print '----------------------------------------------------------------------'
print 'Cleanup'
print '----------------------------------------------------------------------'

for server in cluster:
    server.stop()
    print 'server', server.id, 'done'
print

master.cleanup()
master.deploy()
